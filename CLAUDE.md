...
**目的**:
允许开发者和产品经理强制为所有用户显示新手指引，无视数据库中的完成状态。主要用于：
- 测试新手指引流程变更
- 向利益相关者演示产品
- 录制营销视频展示新手引导体验
- 调试新手指引相关问题
...
NEXT_PUBLIC_FORCE_SHOW_ONBOARDING=true
```

**行为**:
- 设置为 `true`: 每次页面加载时所有用户都会看到新手指引
- 设置为 `false` 或未设置（默认）: 新手指引遵循数据库完成状态
- 用户操作（完成/跳过）仍会更新数据库，但刷新后指引会重新出现
- 环境变量通过 `NEXT_PUBLIC_` 前缀在客户端读取

**实现细节**:
- 覆盖逻辑位于: `hooks/useOnboarding.ts:4-5` (常量定义) 和 `:34-35` (状态覆盖)
- API 获取后覆盖 `completed` 和 `current_step` 字段
- 数据库写操作正常进行（保持数据完整性）
- 通过 `OnboardingTrigger` 组件手动触发功能正常工作

**测试步骤**:
1. 在 `.env.local` 中设置 `NEXT_PUBLIC_FORCE_SHOW_ONBOARDING=true`
2. 重启开发服务器 (`pnpm dev`)
3. 访问 `/dashboard` - 新手指引应该出现
4. 完成或跳过指引
5. 刷新页面 - 指引应该重新出现
...
- 不绕过身份验证或访问控制
- 保持数据库完整性（完成记录仍会保存）
- 建议生产环境保持 `false`，除非正在演示

**技术决策**:
- **为什么用 Hook 层覆盖？** 单一数据源，所有消费者自动继承行为
- **为什么用 `NEXT_PUBLIC_` 前缀？** 客户端组件需要访问，Next.js 只暴露此类变量到浏览器
- **为什么数据库仍然写入？** 保持数据完整性，生产环境下可追踪真实完成状态
