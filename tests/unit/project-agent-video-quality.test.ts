...
test('project agent exposes model-aware quality choices', () => {
  ...
  assert.deepEqual(getProjectAgentAllowedVideoQualities('video_clone', 'wan_27'), ['720p', '1080p']);
  assert.deepEqual(getProjectAgentAllowedVideoQualities('avatar_ads', 'wan_27'), ['720p', '1080p']);
...
test('project agent quality normalization preserves valid choices and falls back when needed', () => {
  ...
  assert.equal(getEffectiveProjectAgentVideoQuality('avatar_ads', 'wan_27', '1080p'), '1080p');
...
test('project agent quality selector is interactive only when multiple choices exist', () => {
  ...
  assert.equal(canChangeProjectAgentVideoQuality('video_clone', 'wan_27'), true);
  assert.equal(canChangeProjectAgentVideoQuality('avatar_ads', 'wan_27'), true);
