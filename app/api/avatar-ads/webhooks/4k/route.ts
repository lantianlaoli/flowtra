...
export async function POST(request: NextRequest) {
  try {
    ...
    // id, project_id, scene_number, video_4k_task_id, video_4k_url, video_4k_webhook_received_at.
    const { data: scene, error: sceneError } = await supabase
      .from('avatar_ads_scenes')
      .select('id, project_id, scene_number, video_4k_url')
    ...
    const { data: scenes } = await supabase
      .from('avatar_ads_scenes')
      .select('id, scene_number, video_4k_url')
      .eq('project_id', scene.project_id)
      .order('scene_number', { ascending: true });

    const allReady = scenes?.every(item => item.video_4k_url) ?? false;
    ...
    console.error('[Avatar Ads 4K Webhook] Error:', error);
