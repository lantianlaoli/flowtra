...
import { AVATAR_ADS_SIMULATION_SEQUENCE } from '@/lib/avatar-ads-tracking';
...
export async function GET(
  ...
  try {
    ...
    const latestStepIndex = AVATAR_ADS_SIMULATION_SEQUENCE.findIndex(
      (entry) => entry.status === progressReferenceStatus
    );

    const stageSnapshot = AVATAR_ADS_SIMULATION_SEQUENCE.map((entry, index) => ({
      status: entry.status,
