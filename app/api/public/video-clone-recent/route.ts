...
interface VideoCloneProjectRow {
  id: string;
  created_at: string;
...
function mapProjectRow(row: VideoCloneProjectRow): VideoCloneProject {
  return {
    id: row.id,
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      console.error('Failed to fetch recent Video Clone projects:', error);
      return NextResponse.json({ success: false, error: 'Failed to fetch recent Video Clone projects' }, { status: 500 });
    }
    ...
    console.error('Unexpected error fetching public Video Clone projects:', error);
