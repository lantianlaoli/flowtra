...
export async function POST(request: NextRequest) {
  try {
    ...
    // id, project_id, segment_index, video_1080p_task_id, video_1080p_url, video_1080p_webhook_received_at.
    const { data: segment, error: segmentError } = await supabase
      .from('competitor_ugc_replication_segments')
      .select('id, project_id, segment_index, video_1080p_url')
    ...
    const { data: segments } = await supabase
      .from('competitor_ugc_replication_segments')
      .select('id, segment_index, video_1080p_url')
      .eq('project_id', segment.project_id)
      .order('segment_index', { ascending: true });

    const allReady = segments?.every(item => item.video_1080p_url) ?? false;
    ...
    console.error('[UGC 1080p Webhook] Error:', error);
