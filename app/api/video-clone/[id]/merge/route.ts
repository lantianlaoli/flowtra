...
import { getSupabaseAdmin, type VideoCloneSegment } from '@/lib/supabase';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { taskId } = await mergeVideosWithFal(
      (segments as VideoCloneSegment[]).map(seg => seg.video_url as string),
      aspectRatio,
      '/api/video-clone/webhooks/merge' // Event-driven: Register webhook for instant merge completion
    );
