...
/**
 * POST /api/reference-videos/upload-temp
 *
 * Temporarily uploads a reference video file to storage WITHOUT creating a database record.
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[POST /api/reference-videos/upload-temp] Uploading temp file for analysis: ${adFile.name}`);

    const filePath = buildReferenceVideoTempUploadPath({
      userId,
    ...
    if (error) {
      console.error(`[POST /api/reference-videos/upload-temp] Storage upload error:`, error);
    ...
    console.log(`[POST /api/reference-videos/upload-temp] ✅ Temp file uploaded: ${ref.publicUrl}`);
    ...
    console.error('[POST /api/reference-videos/upload-temp] Unexpected error:', error);
    return NextResponse.json(
      {
