import type { ImageCloneBulkRow, ImageCloneBulkWorkbook } from "@/lib/image-clone-bulk-types";

export function buildImageCloneBulkPrompt(workbook: ImageCloneBulkWorkbook, row: ImageCloneBulkRow) {
  ...
  return [
    ...
    "- Translate non-English marketing copy into concise natural English unless the row explicitly asks to preserve non-English text.",
    "- Keep on-image text short, readable, correctly spelled, and integrated into the layout.",
    "- Use a single consistent font family across all text elements.",
    "",
    "Output requirements:",
