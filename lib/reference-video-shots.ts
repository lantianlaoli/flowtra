...
export interface ReferenceVideoTimeline {
...
export function parseReferenceVideoTimeline(
  ...
  const shots: ReferenceVideoShot[] = [];
...
export function getReferenceVideoShotCount(
  analysisResult: JsonRecord | null | undefined
): number {
  return getAnalysisShotCount(analysisResult);
}

export function sumShotDurations(shots: ReferenceVideoShot[]): number {
