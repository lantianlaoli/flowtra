...
import {
  ReferenceVideoShotForm,
  createEmptyShot,
  reindexShots
} from '@/lib/reference-video-shot-form';

interface ReferenceVideoShotsEditorProps {
  shots: ReferenceVideoShotForm[];
  onShotsChange: (shots: ReferenceVideoShotForm[]) => void;
  title?: string;
  description?: string;
  showSummary?: boolean;
  readOnly?: boolean;
...
export default function ReferenceVideoShotsEditor({
  shots,
  onShotsChange,
  title,
  description,
...
}: ReferenceVideoShotsEditorProps) {
  ...
  const updateShot = <K extends keyof ReferenceVideoShotForm>(shotId: number, key: K, value: ReferenceVideoShotForm[K]) => {
    if (readOnly) return;
