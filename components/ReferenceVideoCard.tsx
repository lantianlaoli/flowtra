...
interface ReferenceVideoCardProps {
  ...
  onEdit: (referenceVideo: ReferenceVideo) => void;
  onDelete: (id: string) => void;
  onSelect?: (referenceVideo: ReferenceVideo) => void;
  isSelected?: boolean;
  isDeleting?: boolean;
  mode?: 'manage' | 'select';
}
...
export default function ReferenceVideoCard({
  ...
  const languageDisplay = getLanguageDisplayInfo(referenceVideo.language);
  const analysisStatus = referenceVideo.analysis_status || 'pending';
  const analysis = (referenceVideo.analysis_result || null) as Record<string, unknown> | null;
          ...
          <div className="flex gap-3">
            <div className="h-20 w-14 rounded-md bg-gray-100 border border-gray-200 overflow-hidden flex items-center justify-center">
              {previewVideoUrl ? (
                ...
                <Image
                  src={previewCoverUrl}
                  alt={referenceVideo.reference_name}
                  width={56}
                  height={80}
                  className="h-full w-full object-cover"
                />
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-black truncate">
                {referenceVideo.reference_name}
              </p>
              <div className="flex flex-wrap gap-3 text-[11px] text-gray-500 mt-2">
                <span className="inline-flex items-center gap-1">
                  <Clock3 className="w-3 h-3" />
                  {referenceVideo.video_duration_seconds ? `${referenceVideo.video_duration_seconds}s` : '--'}
                </span>
                <span className="inline-flex items-center gap-1">
    ...
    >
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs pt-1">
          {referenceVideo.video_duration_seconds && (
            <span className="inline-flex items-center gap-1 px-2 py-1 rounded border border-[#E5E5E5] bg-white text-black font-medium text-[10px] uppercase tracking-wide">
              ...
              {referenceVideo.video_duration_seconds}s
            </span>
          )}
          {languageDisplay && (
            <span className="inline-flex items-center gap-1 px-2 py-1 rounded border border-[#E5E5E5] bg-white text-black font-medium text-[10px] uppercase tracking-wide">
