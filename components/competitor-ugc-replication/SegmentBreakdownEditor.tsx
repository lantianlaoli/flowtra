  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <button
          type="button"
          className="group inline-flex items-center gap-2.5 rounded-lg border border-[#E5E5E5] bg-white px-4 py-3 text-sm font-medium text-black transition-all hover:border-black hover:shadow-sm"
        >
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-[calc(100%-2rem)] max-w-4xl translate-x-[-50%] translate-y-[-50%] bg-white shadow-2xl data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] overflow-hidden" style={{ borderRadius: '8px', maxHeight: 'calc(100vh - 4rem)' }}>
          {/* Header */}
          <div className="flex items-center justify-between border-b border-[#E5E5E5] px-8 py-6">
            <div className="space-y-1">
              ...
              <Dialog.Description className="text-sm text-[#666666]">
            ...
            <Dialog.Close asChild>
              <button
                type="button"
                className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-[#E5E5E5] text-[#666666] transition-colors hover:border-black hover:text-black"
                aria-label="Close"
              >
                <X className="h-4 w-4" />
              </button>
          ...
          {/* Scrollable Content */}
          <div className="overflow-y-auto px-8 py-6" style={{ maxHeight: 'calc(100vh - 12rem)' }}>
            <div className="space-y-3">
              {segments.map((segment, index) => (
                <button
                  ...
                  className="group w-full rounded-lg border border-[#E5E5E5] bg-[#F7F7F7] p-5 text-left transition-all hover:border-black hover:bg-white hover:shadow-sm"
                >
                  <div className="flex items-start gap-4">
                    {/* Segment Number */}
                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-white border border-[#E5E5E5] text-sm font-semibold text-black">
                      {index + 1}
                    </div>
                    ...
                    <div className="min-w-0 flex-1 space-y-2">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-medium ${getStatusColor(segment.status)}`}>
                            {formatStatus(segment.status)}
                          </span>
                          <ChevronRight className="h-4 w-4 text-[#666666] transition-transform group-hover:translate-x-0.5" />
                        </div>
                      </div>
          ...
          <div className="border-t border-[#E5E5E5] bg-[#F7F7F7] px-8 py-4">
            <div className="flex items-center justify-between">
              ...
              <Dialog.Close asChild>
                <button
                  type="button"
                  className="inline-flex items-center gap-2 rounded-lg bg-black px-4 py-2 text-sm font-medium text-white transition-opacity hover:opacity-90"
                >
                  Done
                </button>
              </Dialog.Close>
