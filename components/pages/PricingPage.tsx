...
const pricingPlanItems = {
  lite: [
    { key: 'credits', content: () => <><span className="font-bold text-gray-900">1,250</span> credits/month</> },
    ...
    { key: 'viral', content: () => 'Video Clone' },
    { key: 'avatar', content: () => 'Avatar Ads' },
    { key: 'motion', content: () => 'Motion Clone' },
    { key: 'tiktok', content: () => 'TikTok publishing support' },
  ],
  plus: [
    { key: 'credits', content: () => <><span className="font-bold text-gray-900">1,930</span> credits/month</> },
    { key: 'videos', content: (videos: number) => <>≈ <span className="font-bold text-gray-900">{videos}</span> Seedance 2 Fast videos</> },
    {
      key: 'agent',
      content: () => (
...
} satisfies Record<'lite' | 'plus' | 'basic' | 'pro', { key: string; content: (videos: number) => ReactNode }[]>;
...
export default function PricingPage() {
  ...
  const plusVideos = Math.floor(900 / CREDIT_COSTS.seedance_2_fast);
  ...
  const handlePurchase = async (packageName: 'lite' | 'plus' | 'basic' | 'pro') => {
    if (!isLoaded) {
  ...
  const PricingButton = ({ packageName }: { packageName: 'lite' | 'plus' | 'basic' | 'pro' }) => {
    if (!isLoaded) {
      return (
        <button
          disabled
    ...
    if (user) {
      ...
      const buttonClass = packageName === 'basic' || packageName === 'plus'
        ...
        <div className="py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 md:gap-8 max-w-6xl mx-auto">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 md:p-8 shadow-sm hover:border-gray-300 transition-colors flex flex-col">
              ...
              <ul className="space-y-2.5 mb-6 md:mb-8 flex-grow">
                {pricingPlanItems.plus.map((item) => (
                  <li key={item.key} className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-gray-600 rounded-full"></div>
                    <span className="flex-1 text-gray-600">{item.content(plusVideos)}</span>
                  </li>
                ))}
              </ul>
            ...
            {/* Ultra Plan */}
            <div className="bg-white rounded-2xl border border-gray-200 p-6 md:p-8 shadow-sm hover:border-gray-300 transition-colors flex flex-col">
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Ultra</h3>
              <div className="text-3xl font-bold text-gray-900 mb-4">
                $99
                <span className="text-lg font-normal text-gray-600">
                  /month
