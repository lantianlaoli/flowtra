...
interface ApiResponse {
  success: boolean;
  projects?: VideoCloneProject[];
  error?: string;
}

export function VideoCloneRecentList() {
  const [projects, setProjects] = useState<VideoCloneProject[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadProjects() {
      try {
        ...
        const response = await fetch('/api/public/video-clone-recent', {
          method: 'GET',
          cache: 'no-store'
        });
  ...
  if (loading) {
    return (
      <div className="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-neutral-600">Loading recent Video Clone projects…</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 p-6 shadow-sm">
        <p className="text-sm font-medium text-red-700">Failed to load Video Clone projects</p>
  ...
  if (projects.length === 0) {
    return (
      <div className="rounded-lg border border-neutral-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-neutral-600">No Video Clone projects found yet.</p>
      </div>
    );
  }
...
export default VideoCloneRecentList;
