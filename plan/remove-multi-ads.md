...
- `components/ui/ShowcaseSection.tsx` accepts `workflowType: 'competitor-ugc-replication' | 'multi-variant-ads' | 'character-ads'` and hits `/api/multi-variant-ads/showcase` when that type is selected. Update the type unions, API selection logic, and UI copy so only the remaining workflows display.
...
- `app/api/multi-variant-ads/ad-copy/route.ts` and `app/api/multi-variant-ads/watermark/route.ts` re-export the competitor-ugc-replication handlers for parity; these can drop once no clients depend on them.

## 4. Shared data surfaces that mix multi-variant items with other features
- `app/api/history/route.ts` merges Competitor UGC Replication, Multi-Variant, Character, and Watermark Removal projects and exposes `adType: 'multi-variant'`. Remove the multi-variant query block, type definitions, and union members.
...
- `app/api/recent-videos/route.ts` compares the latest `competitor_ugc_replication_projects` and `multi_variant_ads_projects` rows; remove the multi-variant fetch and simplify the response shape.
...
- `app/api/tiktok/publish/init/route.ts` attempts to find a video by ID inside `multi_variant_ads_projects` if it wasn’t found in Competitor UGC Replication. Update the fallback logic and associated user messaging.
...
1. Removing the feature makes the `/dashboard/multi-variant-ads` route invalid. Decide whether we want to 301 any existing links to `/dashboard/competitor-ugc-replication` or show an informative fallback before deleting the redirect.
